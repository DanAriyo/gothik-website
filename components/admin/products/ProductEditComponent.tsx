...
export default function ProductEditComponent({
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 text-zinc-900">
      {/* HEADER */}
      <div className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-zinc-950">
          Modifica <span className="text-red-600">Prodotto</span>
        </h1>
        <p className="text-xs text-zinc-500 mt-1">
          Aggiorna dettagli, prezzo, sconti o immagini del capo selezionato.
        </p>
      </div>

      <form
        action={updateWithId}
        className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white p-6 sm:p-8 rounded-2xl border border-zinc-200 shadow-sm"
      >
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-6 lg:border-l lg:border-zinc-100 lg:pl-8">
                ...
                >
                  ...
                  <span className="text-[11px] text-zinc-400 font-mono mt-1">
                    {images.length === 0
                      ...
                      : `${images.length} ${images.length === 1 ? "foto presente" : "foto presenti"}`}
                  </span>
                </button>
              )}
            </CldUploadWidget>
          ...
          {/* BOTTONI DI AZIONE */}
          <div className="pt-4 border-t border-zinc-100 lg:border-t-0 lg:pt-0 flex flex-col sm:flex-row gap-3">
            <Link
              href="/admin/products"
              className="flex-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 hover:text-zinc-900 font-mono font-bold py-3.5 px-4 rounded-xl transition-colors duration-200 uppercase text-center text-xs tracking-wider flex items-center justify-center gap-2"
            >
